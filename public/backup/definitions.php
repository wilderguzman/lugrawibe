...
// definitions
define('PMBP_VERSION',"2.5");  // this is the version of this LugrawibeBackup release
...
define('PMBP_WEBSITE',"http://www.LugrawibeBackup.net");
...
if (@include_once($prepath.PMBP_GLOBAL_CONF_MU)) {
    if ($PMBP_MU_CONF['sql_user_admin'] && $PMBP_MU_CONF['sql_passwd_admin']) {
        ...
        if(!mysql_close($con)) {
            echo "LugrawibeBackup stoped loading due to security reasons: The MySQL connection could not be shut down! (".__FILE__.", Line ".__LINE__.")";
            exit;
        }
